...
class statCatScorePredictor_exact:
    ...
    lutCols = ['c'+str(ii) for ii in range(MAX_TURNS)]  # lookuptable columns
    ...
    def predict(self, dice):
        ...
        if not comp in self.lookupTable.index:
            ...
            # この先のバージョンのpandasの場合は下記で
            #self.lookupTable = pd.concat([self.lookupTable.concat,dfTmp])
...
class AbstractPlayer(ABC):  # abstract class
    ...
    def benchmark(self, nGames=100, seed=None):
        ...
        for ii in range(nGames):
            game = Game(self)
            scores += [game.log.loc[MAX_TURNS, 'scoreBoard'].getSum()]
...
class PlayerAI_oneShot_test(AbstractPlayer):
    ...
    def predict_score(self, scoreBoard):
        ...
        if False:  # FOR DEVELOPMENT ONLY: calc weights
            sb = ScoreBoard()
            weights = np.empty(shape=(MAX_TURNS))
...
#        """
#        games = list_cast(games)
#        scoreBoards = [game.log[rr][0] for game in games for rr in range(MAX_TURNS)]
#        dices = [game.log[rr][-2] for game in games for rr in range(MAX_TURNS)]
#        cats = [game.log[rr][-1] for game in games for rr in range(MAX_TURNS)]
...
#        x[:MAX_TURNS] = scoreBoard.scores.data / 50  # scores
#        x[MAX_TURNS:(MAX_TURNS*2)] = scoreBoard.scores.mask.astype(int)  # avail. cats
#        x[(MAX_TURNS*2):31] = (dice.vals -1) / 5  # dice
#        x[31] = cat
        return x
...
class PlayerAI_full_v0(AbstractPlayer):
    ...
    def __init__(
            ...
            lenScrReplayMem=MAX_TURNS*100, lenScrMiniBatch=MAX_TURNS*50,
            rrRgrArgs={'hidden_layer_sizes':(40, 40, 40, 40, 40, 10)},
            lenRrReplayMem=(MAX_TURNS*2)*100, lenRrMiniBatch=(MAX_TURNS*2)*50,
            nIterPartFit=5,
            gamma=1,
            fn=None):
        """
...
#        lp(opts)
    ...
    def encode_scrRgr_x(self, scoreBoard):
        ...
        x[:MAX_TURNS] = scoreBoard.mask.astype(int)
        x[MAX_TURNS] = scoreBoard.getUpperSum() / 63
...
#        upSum = scoreBoard.getUpperSum()
#        if upSum >= 63 or np.sum(scoreBoard.mask[:6])==0:
##            x[MAX_TURNS] = (5*(2+3+4+5+6) - 63) / 5  # max posible values
#            x[MAX_TURNS] = -63/5  # min possible value
#        else:
#            # nbp: needed bonus progress (i.e. 3 in each cat)
#            nbp = np.sum(
#                    np.logical_not(scoreBoard.mask[:6]).astype(int)
...
#            x[MAX_TURNS] = -63/5
    ...
    def encode_SC(self, scoreBoard):
        ...
        x[:MAX_TURNS] = scoreBoard.mask.astype(int)
        x[MAX_TURNS] = self.encode_bonus(scoreBoard)
        return x
    ...
    def nFeat_Rr(self):
        ...
        Directly coupled to self.encoder.
        """
        return MAX_TURNS + MAX_TURNS + 1 + 1
    def encode_Rr(self, scoreBoard, attempt, dice, deciRr):
        """Encodes a scoreboard to a numpy array,
        which is used as the input layer
        MAX_TURNS: open cats
        MAX_TURNS: exp score in each cat
        1: attempt
        1: bonus
        """
        x = np.zeros(shape=(self.nFeat_Rr))
        x[:MAX_TURNS] = scoreBoard.mask.astype(int)
        x[MAX_TURNS:(MAX_TURNS*2)] = self.predict_Ex(dice, deciRr)
        x[MAX_TURNS:(MAX_TURNS*2)] *= x[:MAX_TURNS]
#        x[MAX_TURNS:26], _ = ScoreBoard.stat_cat_score(dice.keep(deciRr))
        x[24] = attempt
        x[25] = self.encode_bonus(scoreBoard)
        return x
    ...
    def encode_Ex(self, dice, deciRr):
        ...
        for ii in range(1, 6):  # number of single, double, ...
            x[12 + ii] = len(hist[hist == ii])  # x[MAX_TURNS]-x[17]
        # small straight
...
#            if np.sum(deciRr) == 0:
#                nMC = 1
#            yMC = np.empty(shape=(nMC, MAX_TURNS))
#            for mm in range(nMC):
...
#        return np.mean(bmAbs, axis=0), np.std(benchmark, axis=0)
    ...
    def to_repMem(self, gameLog):
        ...
        self.repMemSC = self.repMemSC[-self.nGamesPreplayMem*MAX_TURNS:]
        self.repMemRr = self.repMemRr[-self.nGamesPreplayMem*(MAX_TURNS*2):]
        self.repMemEx = self.repMemEx[-self.nGamesPreplayMem*(MAX_TURNS*2):]
...
#                    opts = np.array(opts)
            ...
            #rgrSC
            n_samples = self.nGamesPartFit * MAX_TURNS
...
#                self.rgrSC = self.rgrSC.partial_fit(X, y)
            ...
            #rgrEx
            n_samples = self.nGamesPartFit * (MAX_TURNS*2)
            X = np.empty(shape=(n_samples, self.nFeat_Ex))
            y = np.empty(shape=(n_samples, MAX_TURNS))
...
#            x[MAX_TURNS] = -63/5
    ...
    def encode_Rr(self, scoreBoard, attempt, dice, deciRr):
        ...
        x[:MAX_TURNS] = scoreBoard.mask.astype(int)
#        x[MAX_TURNS:(MAX_TURNS*2)] = self.predict_Ex(dice, deciRr)
        x[MAX_TURNS:MAX_TURNS*2] = self.scsp.predict(dice.keep(deciRr))
        x[MAX_TURNS:MAX_TURNS*2] *= x[:MAX_TURNS]
#        x[MAX_TURNS:(MAX_TURNS*2)], _ = ScoreBoard.stat_cat_score(dice.keep(deciRr))
        x[24] = attempt
        x[25] = self.encode_bonus(scoreBoard)
        return x
    def predict_Rr(self, scoreBoard, attempt, dice, deciRr):
